...
def train_ensemble_model(df, target_col):
    ...
    # Import all required libraries INSIDE the function to avoid scope issues
    ...
    with st.spinner("🚀 Training AI Ensemble Model..."):
        ...
        try:
            ...
            st.warning(f"XGBoost not available: {str(e)[:100]}")
        ...
        try:
            ...
            st.warning(f"LightGBM not available: {str(e)[:100]}")
            lgb_working = False
        ...
        metrics = {
            ...
            'Voting Ensemble': calc_metrics(y_test, y_pred, y_proba),
        }
        ...
        try:
            ...
            shap_bar_buf = shap_swarm_buf = shap_waterfall_buf = None
...
def generate_pdf_report(cleaning_df, stats_df, metrics, best_model, attrition_rate, cleaning_insights, pred_result=None, cv_scores=None):
    ...
    # Get the best model metrics safely
    # Get the best model metrics safely with error handling
    try:
...
if 'df' in st.session_state and 'target' in st.session_state and st.session_state['df'] is not None:
    ...
    # Calculate attrition rate - FIXED (INDENTED PROPERLY)
    try:
        if df[target].dtype == 'object':
        ...
        attrition_rate = 0.5  # Default fallback
    ...
    # ... rest of your code continues with same indentation level
            ...
            with st.spinner("Generating professional HR report..."):
                try:
                    pdf_buffer = generate_pdf_report(
                        ...
                        metrics,
                        st.session_state.get('best_model', 'Voting Ensemble'),
                        attrition_rate,
